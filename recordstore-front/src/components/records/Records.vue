  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form action @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <p class="pt-4">
          Don't see an artist?
          <router-link class="text-grey-darker underline" to="/artists">Create one</router-link>
        </p>
      </div>
  ...
  created() {
    if (!localStorage.signedIn) {
      ...
      this.$http.secured
        ...
        .catch(error =>
          this.setError(error, "Something went wrong with records")
        );
      ...
      this.$http.secured
        ...
        .catch(error =>
          this.setError(error, "Something went wrong with artists")
        );
    }
  },
  methods: {
